<script>
  ...
  // Handler de DataTable — se actualiza cuando cambia `usuarios`
  let handler = $derived(new DataHandler(usuarios, { rowsPerPage: 10 }))
      ...
      <div class="table-container">
        <Datatable {handler}>
          <table>
            <thead>
              <tr>
                <Th {handler} orderBy="id_usuario">ID</Th>
                <Th {handler} orderBy="nombre">Usuario</Th>
                <Th {handler} orderBy="cedula">Identificación</Th>
                <Th {handler} orderBy="correo">Correo</Th>
                <Th {handler} orderBy="id_rol">Rol</Th>
                <th class="text-center">Acciones</th>
              </tr>
              <tr>
                <ThFilter {handler} filterBy="id_usuario" />
                <ThFilter {handler} filterBy="nombre" />
                <ThFilter {handler} filterBy="cedula" />
                <ThFilter {handler} filterBy="correo" />
                <ThFilter {handler} filterBy="id_rol" />
                <th></th>
              </tr>
            </thead>
            <tbody>
...
<style>
  ...
  /* Tabla */
  .table-container { background: white; border-radius: 20px; border: 1px solid #e2e8f0; overflow-x: auto; box-shadow: 0 4px 6px rgba(0,0,0,0.02); padding: 16px; }
