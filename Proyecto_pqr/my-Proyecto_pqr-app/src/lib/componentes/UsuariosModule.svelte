<script>
  ...
  function defaultForm() {
    return {
      ...
      semestre: '', cargo: '', celular: '', correo: '', id_rol: ''
    }
  }
  ...
  async function saveUsuario() {
    if (!form.nombre || !form.cedula || !form.correo) {
      showToast('Nombre, cédula y correo son requeridos', 'error')
      return
    }
    saving = true
    try {
...
<div class="module">
  ...
  <div class="page-header">
    ...
    <div class="header-actions">
      ...
      {:else}
        <button class="btn-primary" on:click={openCreate}>＋ Nuevo Usuario</button>
      {/if}
    </div>
  </div>
  ...
  {#if view === 'list'}
    ...
    {:else if filtered.length === 0}
      <div class="empty-state">
        ...
        <button class="btn-primary" on:click={openCreate}>Crear usuario</button>
      </div>
    {:else}
              ...
              <tr>
                ...
                <td>
                  <div class="row-actions">
                    <button class="icon-btn edit" title="Editar" on:click={() => openEdit(u)}>✎</button>
                    <button class="icon-btn delete" title="Eliminar" on:click={() => deleteUsuario(u.id_usuario)}>✕</button>
                  </div>
                </td>
              </tr>
            {/each}
    ...
    <div class="form-card">
      <div class="form-grid">
        <div class="field">
          ...
          <input type="text" bind:value={form.nombre} placeholder="Nombre del usuario" />
        </div>
        <div class="field">
          <label>Cédula <span class="req">*</span></label>
          <input type="text" bind:value={form.cedula} placeholder="Número de cédula" />
        </div>
        <div class="field">
          <label>Correo electrónico <span class="req">*</span></label>
          <input type="email" bind:value={form.correo} placeholder="correo@ejemplo.com" />
        </div>
        <div class="field">
          <label>Celular</label>
          <input type="text" bind:value={form.celular} placeholder="Número de celular" />
        </div>
        <div class="field">
          <label>Carrera</label>
          <input type="text" bind:value={form.carrera} placeholder="Carrera o programa" />
        </div>
        <div class="field">
          <label>Semestre</label>
          <input type="number" bind:value={form.semestre} placeholder="Semestre actual" min="1" max="12" />
        </div>
        <div class="field">
          <label>Cargo</label>
          <input type="text" bind:value={form.cargo} placeholder="Cargo en la institución" />
        </div>
        <div class="field">
          <label>Rol</label>
      ...
      <div class="form-actions">
        ...
        <button class="btn-primary" on:click={saveUsuario} disabled={saving}>
...
<style>
  ...
  .toast {
    ...
    font-size: 14px; font-weight: 500; animation: slideIn 0.3s ease; max-width: 360px;
  }
  ...
  .search-input {
    ...
    padding: 10px 12px 10px 38px; font-size: 14px; outline: none; transition: border-color 0.2s;
  }
  ...
  th {
    ...
    letter-spacing: 0.05em; font-weight: 600; border-bottom: 1px solid var(--border); white-space: nowrap;
  }
  ...
  .avatar-sm {
    ...
    background: rgba(45,45,58,0.1); color: var(--accent);
  ...
  .btn-primary {
    ...
    display: flex; align-items: center; gap: 6px; transition: opacity 0.2s, transform 0.15s;
  }
  ...
  .loading-state, .empty-state {
    ...
    justify-content: center; padding: 80px 40px; gap: 16px; color: var(--text-muted);
  }
