<script>
  ...
  function defaultForm() {
    formNombre = ''; formCedula = ''; formCorreo = ''
    formIdRol  = ''; formCarrera = ''; formCelular = ''; formSemestre = ''
  }

  function initTable(data) {
    handler = new TableHandler(data, { rowsPerPage: 10 })
  }
  ...
  onMount(async () => {
    ...
    try {
      ...
      const rawUsuarios = uData.value?.resultado || uData.value || []
      const rawRoles    = rData.value?.resultado || rData.value || []
      usuarios = Array.isArray(rawUsuarios) ? [...rawUsuarios] : Object.values(rawUsuarios)
      roles    = Array.isArray(rawRoles)    ? [...rawRoles]    : Object.values(rawRoles)
      initTable([...usuarios])
  ...
  async function saveUsuario() {
    ...
    try {
      ...
      const raw = res.resultado || res || []
      usuarios = Array.isArray(raw) ? [...raw] : Object.values(raw)
      initTable([...usuarios])
      view = 'list'
  ...
  {#if view === 'list'}
    <div class="toolbar">
      ...
      {#if handler}
        <div class="rows-per-page">
          <label>Filas:</label>
          <select onchange={(e) => handler.setRowsPerPage(parseInt(e.target.value))}>
    ...
    {#if handler}
      <div class="results-count">
        {handler.rowCount.total} {handler.rowCount.total === 1 ? 'usuario encontrado' : 'usuarios encontrados'}
      </div>
    {/if}
...
<th onclick={() => sortBy('id_rol')} class="sortable" class:active={sortField==='id_rol'}>
  Rol {@render sortIcon('id_rol')}
          ...
          <tbody>
            {#each handler.rows as u (u.id_usuario)}
              <tr>
                <td><span class="id-tag">#{u.id_usuario}</span></td>
                <td>
                  <Avatar nombre={u.nombre} size="md" showName={true} rol={u.carrera || 'CUL'} />
                </td>
        ...
        <div class="pagination">
          <button class="page-btn" onclick={() => handler.setPage(1)} disabled={handler.currentPage === 1}>««</button>
          <button class="page-btn" onclick={() => handler.setPage('previous')} disabled={handler.currentPage === 1}>‹ Anterior</button>
          <span class="page-info">Página <strong>{handler.currentPage}</strong> de <strong>{handler.pageCount}</strong></span>
          <button class="page-btn" onclick={() => handler.setPage('next')} disabled={handler.currentPage === handler.pageCount}>Siguiente ›</button>
          <button class="page-btn" onclick={() => handler.setPage('last')} disabled={handler.currentPage === handler.pageCount}>»»</button>
        </div>
      </div>
    {/if}
    ...
    <div class="center-container">
      <div class="card animate-up">
        <div class="form-padding">
          ...
          <FormField label="Nombre Completo" tipo="text" bind:valor={formNombre} placeholder="Ej: Juan Pérez García" required={true} />

          <div class="field-group">
            <FormField label="Cédula" tipo="text" bind:valor={formCedula} placeholder="100000001" required={true} />
            <FormField label="Correo" tipo="email" bind:valor={formCorreo} placeholder="usuario@cul.edu.co" required={true} />
          </div>
          ...
          <div class="field-group">
            ...
            />
            <FormField label="Carrera" tipo="text" bind:valor={formCarrera} placeholder="Ej: Ingeniería de Sistemas" />
          </div>

          <div class="field-group">
            <FormField label="Celular" tipo="text" bind:valor={formCelular} placeholder="3001234567" />
            <FormField label="Semestre" tipo="number" bind:valor={formSemestre} placeholder="1 – 10" />
          </div>
...
<style>
  ...
  .rows-per-page select { border: 1.5px solid #e2e8f0; border-radius: 8px; padding: 6px 10px; font-size: 13px; font-family: inherit; outline: none; cursor: pointer; }
