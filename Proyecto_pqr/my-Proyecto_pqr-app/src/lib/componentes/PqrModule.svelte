...
<div class="module">
  ...
  <header class="page-header">
    <div class="header-content">
      ...
      <p class="subtitle">{isAdmin ? 'Administra todas las solicitudes del sistema' : 'Gestiona y consulta el estado de tus trámites'}</p>
    </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              {#if isAdmin}<th>Usuario</th><th>Departamento</th><th>Prioridad</th><th>Estado</th>{/if}
          ...
          <tbody>
            ...
            {#if filtered.length === 0}
              <tr><td colspan={isAdmin ? 9 : 5} class="empty-row">No se encontraron solicitudes</td></tr>
            {/if}
          </tbody>
        </table>
      </div>
